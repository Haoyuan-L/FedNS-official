...
def create_black_patch(x, severity, img_shape=(32, 32, 3)):
    """
    Create a black patch with the same size and dimensions as the input image.
    """
    return np.full(img_shape, 0, dtype=np.uint8)

def create_gaussian_noise_patch(x, severity, img_shape=(32, 32, 3), mean=0, std=1.0):
    """
    Create a patch of Gaussian noise with the same size and dimensions as the input image using scikit-image.
    """
    # Create a blank image
    image = np.zeros(img_shape, dtype=np.float32)
    # Add Gaussian noise
    noise_patch = util.random_noise(image, mode='gaussian', mean=mean, var=std**2)
    # Normalize and convert to uint8
    noise_patch = np.clip(noise_patch * 255, 0, 255).astype(np.uint8)
    return noise_patch

def create_stylegan_patch(x, severity, data_dir="../../stylegan-transform-cifar10", img_shape=(32, 32, 3)):
    """ 
    Create a patch of StyleGAN image with the same size and dimensions as the input image.
    """
    # Assume target_shape is in the form of (height, width, channels)
    target_height, target_width, _ = img_shape

    # Get a list of all files in the data directory
    all_files = [os.path.join(dp, f) for dp, dn, filenames in os.walk(data_dir) for f in filenames if os.path.splitext(f)[1].lower() in ['.jpg', '.jpeg', '.png']]
    if not all_files:
    ...
    img = img.resize((target_width, target_height))  # PIL resize accepts (width, height)
    img_np = np.array(img)
    if img_np.ndim == 2:  # If the image is grayscale, convert it to RGB
        img_np = np.stack((img_np,) * 3, axis=-1)
    elif img_np.shape[2] > 3:  # If the image has an alpha channel, remove it
        img_np = img_np[:, :, :3]
