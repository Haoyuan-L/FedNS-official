...
# Make sure to download all necessary packages, I suggest to create a new conda environment for Pytorch 
# and download all these packages under that environment to avoid any issues related to CUDA.
...
def dummy_transform(x, severity=None):
    x = np.array(x)
    return x


def create_mixed_dataset(data, labels, noise_dict, clean_ratio, severity):
    """
    Create a mixed dataset with a specified ratio of clean and noisy images.
    
    noise_dict: Dictionary of noise functions.
    clean_ratio: Ratio of clean images in the dataset.
    severity: Severity level for the noise.
    
    return: Tuple of mixed data and labels.
    """
    num_samples = len(data)
    num_clean = int(num_samples * clean_ratio)
    clean_data = data[:num_clean]
    clean_labels = labels[:num_clean]

    noisy_data, noisy_labels = [], []
    for img, label in zip(data[num_clean:], labels[num_clean:]):
        noise_type = np.random.choice(list(noise_dict.keys()))
        noisy_img = np.uint8(noise_dict[noise_type](convert_img(img), severity))
        noisy_data.append(noisy_img)
        noisy_labels.append(label)

    combined_data = clean_data + noisy_data
    combined_labels = clean_labels + noisy_labels
...
severity = 5
train_dir = './fmnistdata/train'
valid_dir = './fmnistdata/valid'
test_dir = './fmnistdata/test'
os.makedirs(train_dir, exist_ok=True)
os.makedirs(valid_dir, exist_ok=True)
os.makedirs(test_dir, exist_ok=True)
train_dataset = datasets.FashionMNIST('./fmnistdata', download=True, train=True)
test_dataset = datasets.FashionMNIST('./fmnistdata', download=True, train=False)
convert_img = transforms.Compose([transforms.ToTensor(), transforms.ToPILImage()])

validation_size = 0.2
train_size = len(train_dataset) - int(len(train_dataset) * validation_size)
# Split the dataset
train_set, validation_set = random_split(train_dataset, [train_size, int(len(train_dataset) * validation_size)])

# Normal train data
dataset_path = f'./fmnistdata/train/fmnistdata_original_none_train.pkl'
with open(dataset_path, 'wb') as file:
    pickle.dump([(np.uint8(convert_img(img)), label) for img, label in train_set], file)

# Normal valid data
dataset_path = f'./fmnistdata/valid/fmnistdata_original_none_valid.pkl'
with open(dataset_path, 'wb') as file:
    pickle.dump([(np.uint8(convert_img(img)), label) for img, label in validation_set], file)

# Normal test data
dataset_path = f'./fmnistdata/test/fmnistdata_test.pkl'
with open(dataset_path, 'wb') as file:
    pickle.dump([(np.uint8(convert_img(img)), label) for img, label in test_dataset], file)

# Corrupted train data
for method_name in d.keys():
    print('Creating train images for the corruption ', method_name)
    lvl = {3:'medium', 5:'high'}
    for severity in [3, 5]: # Low, medium, and high severity
        dataset_path = f'./fmnistdata/train/fmnistdata_{method_name}_{lvl[severity]}_train.pkl'
        corruption = lambda clean_img: d[method_name](clean_img, severity)
        with open(dataset_path, 'wb') as file:
            pickle.dump([(np.uint8(corruption(convert_img(img))), label) for img, label in train_set], file)

# Corrupted valid data
for method_name in d.keys():
    print('Creating valid images for the corruption ', method_name)
    lvl = {3:'medium', 5:'high'}
    for severity in [3, 5]: # Low, medium, and high severity
        dataset_path = f'./fmnistdata/valid/fmnistdata_{method_name}_{lvl[severity]}_valid.pkl'
        corruption = lambda clean_img: d[method_name](clean_img, severity)
        with open(dataset_path, 'wb') as file:
            pickle.dump([(np.uint8(corruption(convert_img(img))), label) for img, label in validation_set], file)
