...
severity = 5
train_dir = './pathmnistdata/train'
valid_dir = './pathmnistdata/valid'
test_dir = './pathmnistdata/test'
os.makedirs(train_dir, exist_ok=True)
os.makedirs(valid_dir, exist_ok=True)
os.makedirs(test_dir, exist_ok=True)
train_dataset = PathMNIST(split="train", download=True, root='./pathmnistdata')
val_dataset = PathMNIST(split="val", download=True, root='./pathmnistdata')
test_dataset = PathMNIST(split="test", download=True, root='./pathmnistdata')
convert_img = trn.Compose([trn.ToTensor(), trn.ToPILImage()])

# Normal train data
dataset_path = f'./pathmnistdata/train/pathmnistdata_original_none_train.pkl'
with open(dataset_path, 'wb') as file:
    pickle.dump([(np.uint8(convert_img(img)), label) for img, label in train_dataset], file)

# Normal valid data
dataset_path = f'./pathmnistdata/valid/pathmnistdata_original_none_valid.pkl'
with open(dataset_path, 'wb') as file:
    pickle.dump([(np.uint8(convert_img(img)), label) for img, label in valid_dataset], file)

# Normal test data
dataset_path = f'./pathmnistdata/test/pathmnistdata_test.pkl'
with open(dataset_path, 'wb') as file:
...
for method_name in d.keys():
    ...
    for severity in [3, 5]: # Low, medium, and high severity
        dataset_path = f'./pathmnistdata/train/pathmnistdata_{method_name}_{lvl[severity]}_train.pkl'
        corruption = lambda clean_img: d[method_name](clean_img, severity)
        with open(dataset_path, 'wb') as file:
            pickle.dump([(np.uint8(corruption(convert_img(img))), label) for img, label in train_dataset], file)
...
for method_name in d.keys():
    ...
    lvl = {3:'medium', 5:'high'}
    for severity in [3, 5]: # Low, medium, and high severity
        dataset_path = f'./pathmnistdata/valid/pathmnistdata_{method_name}_{lvl[severity]}_valid.pkl'
        corruption = lambda clean_img: d[method_name](clean_img, severity)
        with open(dataset_path, 'wb') as file:
            pickle.dump([(np.uint8(corruption(convert_img(img))), label) for img, label in val_dataset], file)
