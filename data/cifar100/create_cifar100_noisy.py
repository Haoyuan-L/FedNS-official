...
# We do not need to focus on all these corruptions
...
# Ideally we should generate data for each corruption type, uncomment as necessary
...
d['Elastic'] = elastic_transform
d['Pixelate'] = pixelate
d['JpegCompression'] = jpeg_compression
d['Spatter'] = spatter
d['Saturate'] = saturate

if not os.path.isdir('./cifar100data/subsets'):
    os.mkdir('./cifar100data')
    os.mkdir('./cifar100data/train_labels')
    os.mkdir('./cifar100data/test')

train_data = dset.CIFAR100('./cifar100data/', download=True, train=True) # For CIFAR-100
test_data = dset.CIFAR100('./cifar100data/', download=True, train=False) # For CIFAR-100
# train_data = dset.CIFAR100('<PATH HERE WHERE YOU WANT THE ORGINAL DATA TO BE DOWNLOADED>', train=True)  # For CIFAR-100
...
np.save('./cifar100data/cifar100_original_none.npy', np.array(cifar_c).astype(np.uint8))
np.save('./cifar100data/train_labels/cifar100_train_labels.npy', np.array(labels).astype(np.uint8))
...
for img, label in zip(test_data.data, test_data.targets):
   labels.append(label)
   cifar_c.append(np.uint8(convert_img(img)))
np.save('./cifar100data//test/cifar100_test_images.npy', np.array(cifar_c).astype(np.uint8))
np.save('./cifar100data/test/cifar100_test_labels.npy', np.array(labels).astype(np.uint8))


# Corrupted data
for method_name in d.keys():
    print('Creating images for the corruption', method_name)
    # lvl = {1:'low', 3:'medium', 5:'high'}
    lvl = {3:'medium', 5:'high'}
    for severity in [3, 5]: # Low, medium, and high severity
        ...
        np.save('./cifar100data/cifar100_' + str(method_name) + '_' + lvl[severity] + '.npy', np.array(cifar_c).astype(np.uint8))
