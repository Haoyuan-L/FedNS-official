...
os.mkdir('./cifar10data')
os.mkdir('./cifar10data/train_labels')
os.mkdir('./cifar10data/test')

train_data = dset.CIFAR10('./cifar10data/', download=True, train=True) # For CIFAR-10
test_data = dset.CIFAR10('./cifar10data/', download=True, train=False) # For CIFAR-10
...
np.save('./cifar10data/cifar10_original_none.npy', np.array(cifar_c).astype(np.uint8))
np.save('./cifar10data/train_labels/cifar10_train_labels.npy', np.array(labels).astype(np.uint8))
...
for img, label in zip(test_data.data, test_data.targets):
   labels.append(label)
   cifar_c.append(np.uint8(convert_img(img)))
np.save('./cifar10data//test/cifar10_test_images.npy', np.array(cifar_c).astype(np.uint8))
np.save('./cifar10data/test/cifar10_test_labels.npy', np.array(labels).astype(np.uint8))
...
for method_name in d.keys():
    ...
    for severity in [3, 5]: # Low, medium, and high severity 
        ...
        np.save('./cifar10data/cifar10_' + str(method_name) + '_' + lvl[severity] + '.npy', np.array(cifar_c).astype(np.uint8))
